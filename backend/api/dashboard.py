...
# ✨ --- 디버깅용 프린트 --- ✨
print("✅✅✅ 대시보드 API 라우터(dashboard.py)가 성공적으로 로드되었습니다! ✅✅✅")
...
def test_dashboard_route():
    """라우터가 제대로 연결되었는지 확인하기 위한 간단한 테스트 API"""
    print("🚀 /test-route가 성공적으로 호출되었습니다! 🚀")
    return {"message": "대시보드 라우터 연결 성공!"}
...
def approve_policy_action(
    ...
    # 정책 정보 조회 (위험 그룹 무관하게 ID로만 조회)
    policy = crud.get_action_recommendation_by_id(db, request.policyId)
    if not policy:
    ...
    # 승인 후 high_risk_users 테이블에서 해당 데이터 삭제
    try:
        deleted = crud.delete_high_risk_user(db, request.userId, request.policyId)
        if not deleted:
            print(f"경고: 승인 후 high_risk_users에서 해당 데이터를 찾을 수 없어 삭제하지 못했습니다. (사용자 ID: {request.userId}, 정책 ID: {request.policyId})")
    except Exception as e:
        print(f"오류: 승인 후 high_risk_users 데이터 삭제 실패: {e}")
