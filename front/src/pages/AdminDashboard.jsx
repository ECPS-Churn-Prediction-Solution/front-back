...
const AdminDashboard = () => {
  ...
  const handleReject = async (userId, policyId) => {
    ...
    try {
      ...
      const result = await rejectPolicyAction(
        userId,
        policyId,
        reason || "운영자 거절"
      );
  ...
  const bandPercents = useMemo(() => {
    ...
    bands.forEach((b) => {
      byKey[b.riskBand] = Math.round(b.ratio * 1000) / 10;
    });
    return byKey;
  }, [dashboardData]);

  const donutStyle = useMemo(() => {
    const { VH = 0, H = 0, M = 0, L = 0 } = bandPercents;
    const p1 = VH;
    const p2 = p1 + H;
    const p3 = p2 + M;
    const p4 = p3 + L;
  ...
  const segmentMetrics = useMemo(() => {
    ...
    const maxRate = rates.length ? Math.max(...rates) : 0.2;
        ...
        <>
          <div className="kpi-row">
            <div className="kpi-card">
              ...
              <div className="kpi-value">
                {kpi.customersTotal.toLocaleString()}
              </div>
            ...
            <div className="kpi-card">
              ...
              <div className="kpi-value">
                {kpi.churnCustomers.toLocaleString()}
              </div>
              ...
              <div className="split-left">
                ...
                <div className="risk-distribution">
                  ...
                  <ul className="legend-list">
                    <li className="legend-item vh">
                      <span className="dot" /> Very High Risk{" "}
                      <span className="right">
                        {(bandPercents.VH || 0).toFixed(0)}%
                      </span>
                    </li>
                    <li className="legend-item h">
                      <span className="dot" /> High Risk{" "}
                      <span className="right">
                        {(bandPercents.H || 0).toFixed(0)}%
                      </span>
                    </li>
                    <li className="legend-item m">
                      <span className="dot" /> Medium Risk{" "}
                      <span className="right">
                        {(bandPercents.M || 0).toFixed(0)}%
                      </span>
                    </li>
                    <li className="legend-item l">
                      <span className="dot" /> Low Risk{" "}
                      <span className="right">
                        {(bandPercents.L || 0).toFixed(0)}%
                      </span>
                    </li>
                  </ul>
                </div>
              ...
              <div className="split-right">
                ...
                <div className="trend-chart">
                  <svg viewBox="0 0 360 160" preserveAspectRatio="none">
                    ...
                    {trend && trend.series && trend.series.length > 0 && (
                      <>
                        <path
                          className="vh"
                          d={buildPath(trend.series.map((s) => s.VH || 0))}
                        />
                        <path
                          className="h"
                          d={buildPath(trend.series.map((s) => s.H || 0))}
                        />
                        <path
                          className="m"
                          d={buildPath(trend.series.map((s) => s.M || 0))}
                        />
                        <path
                          className="l"
                          d={buildPath(trend.series.map((s) => s.L || 0))}
                        />
                      </>
                    )}
                  </svg>
                  ...
                  <div className="trend-legend">
                    <span className="vh">
                      <span className="dot" /> 매우 높음
                    </span>
                    <span className="h">
                      <span className="dot" /> 높음
                    </span>
                    <span className="m">
                      <span className="dot" /> 보통
                    </span>
                    <span className="l">
                      <span className="dot" /> 낮음
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="hbar-chart">
                ...
                {segmentMetrics.segments.map((seg) => {
                  ...
                  const width = Math.max(
                    4,
                    Math.round((seg.churnRate / segmentMetrics.maxRate) * 100)
                  );
                  return (
                    <div className="hbar-row" key={seg.bucket}>
                      ...
                      <div
                        ...
                        aria-label={`Churn ${pct.toFixed(1)}%`}
                      >
                        <div
                          className={`hbar-fill ${rateToClass(seg.churnRate)}`}
                          style={{ width: `${width}%` }}
                        />
          ...
          <div className="policy-table-section">
            ...
            <div className="policy-toolbar">
              <div>
                총 {dashboardData?.highRisk?.total?.toLocaleString?.() || 0}건
              </div>
                    ...
                    <tr key={item.userId}>
                      ...
                      <td>
                        <button
                          className="approve-btn"
                          onClick={() =>
                            handleApprove(item.userId, item.action.policyId)
                          }
                          disabled={isProcessing}
                        >
                          {isApproving ? "승인중..." : "승인"}
                        ...
                        <button
                          className="reject-btn"
                          onClick={() =>
                            handleReject(item.userId, item.action.policyId)
                          }
                          disabled={isProcessing}
                        >
                          {isRejecting ? "거절중..." : "거절"}
